...
?>
  ...
  <html><head>
	...
	<script language="javascript">
		...
		function active(maCheck, pObjet) {
			//active ou desactive l'objet pObjet du formulaire selon la valeur de la checkbox
			formRAPPORT_VISITE.elements[pObjet].disabled=!maCheck.checked; 
		}
	</script>
	 <script language="javascript">
        function ajoutLigne( pNumero){//ajoute une ligne de produits/qté à la div "lignes"  
			...
			nouvParagraphe.appendChild(liste) ;
			...
			var qte = document.createElement("label") ;	//crée un label
			nouvParagraphe.appendChild(qte) ;		;				//l'ajoute à la DIV
			qte.innerHTML= "Qte offerte : ";
			
			var qte = document.createElement("input");
			nouvParagraphe.appendChild(qte);
			qte.setAttribute("name","PRA_QTE"+pNumero);
			qte.setAttribute("size","2"); 
			qte.setAttribute("class","zone");
			qte.setAttribute("type","text");
			var bouton = document.createElement("input");
			nouvParagraphe.appendChild(bouton);
  ...
  <div id="contenu">
	  ...
	  <form name="formRAPPORT_VISITE" method="post" action="#">
			...
			?>
			</select><input type="checkbox" name="remplace" value="remplacant" class="zone" onClick="active(this,'RAP_REMPL');active(this,'RAP_REMPL2');"/>Remplacant 
			Nom : <input type="text" name="RAP_REMPL" class="zone" disabled="disabled" />
			Prenom : <input type="text" name="RAP_REMPL2" class="zone" disabled="disabled" /></br>
			
			<label class="titre">DATE :</label><input type="date" size="19" value="<?php echo date('Y-m-d'); ?>" name="RAP_DATE" class="zone" /></br>
			...
			<div class="titre" id="lignes">
				Pas d'&eacutechantillons offerts <input type=checkbox name='PasEch'/>
			...
			while($ligne=mysql_fetch_array($res))
			{
				$nom10=$ligne['MED_NOMCOMMERCIAL'];
				$code10=$ligne['MED_DEPOTLEGAL'];
				?>
				<option value="<?php echo $code10 ?>"><?php echo $nom10 ?></option>
				<?php
			}
			?>
